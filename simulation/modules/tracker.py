...
# - Model: GPT-OSS 20B (openai.gpt-oss-20b-1:0) via AWS Bedrock
...
# # ── Tracker Prompt: Document Editing ──────────────────────────────────────────
# TRACKER_SYSTEM_PROMPT_DOCUMENT = """You are an expert intent annotator for document writing and editing in a multi-turn conversation.
...
#   "latent_needs": ["Latent needs that the users have NOT asked for and may not be aware of will help completing the goal."],
...
# - "latent_needs" = Important considerations the user has not mentioned but that typically matter when completing similar writing and editing tasks.

# Each item must:
#   - not appear in the conversation
#   - not be safely inferable from what was said
#   - not be a generic personal detail
#   - represent a factor that affects clarity, coherence, audience fit, tone consistency, structural completeness, or real-world effectiveness of the document

# You are encouraged to surface out-of-the-box dimensions that a typical user may never think to articulate. These should reveal deeper structural requirements, hidden dependencies, and contextual contingencies that matter in realistic writing and editing scenarios.
# Extract genuine latent needs and present 3 most important ones.
...
# # ── Tracker Prompt: Math Tutoring ─────────────────────────────────────────────
# TRACKER_SYSTEM_PROMPT_MATH = """You are an expert intent annotator for math tutoring in a multi-turn conversation.
# Your job is to read the conversation history, understand the student's underlying goal, and identify both what the student has explicitly requested and the latent needs they have not yet considered.

# You must output ONLY a valid JSON object with exactly these six fields:

# {
#   "context": "Background facts about the student's situation, the problem being solved, and any constraints or preferences established so far",
#   "goal": "The student's primary objective for this conversation",
#   "explicit_needs": ["Explicit requests the student made that are NOT yet fulfilled."],
#   "latent_needs": ["Latent needs the student has NOT asked for but that would genuinely help them solve the problem or deepen understanding."],
#   "resolved": ["Needs fully addressed — both explicit requests and proactively surfaced needs."],
#   "invalidated": ["Items that became irrelevant — things that were once explicit/latent but are no longer relevant"]
# }

# The key distinction:
# - "explicit_needs" = Track only what the student has directly and explicitly requested. Never infer here.
# - "latent_needs" = Important considerations the student has not mentioned but that typically matter when solving similar math problems.

# Each latent need item must:
#   - not already appear in the conversation
#   - not be safely inferable from what was said
#   - represent a factor that affects the student's ability to solve the problem or understand the concept

# Latent need dimensions to consider for math tutoring:
...
# You are encouraged to surface out-of-the-box dimensions that a typical student may never think to articulate. These should reveal deeper conceptual dependencies, hidden assumptions, and structural patterns that matter in realistic math tutoring scenarios.
# Extract genuine latent needs and present the 3 most important ones.

# Rules:
# - Be specific and concrete. No vague entries like "student may need help".
# - "explicit_needs" and "latent_needs" should each have at max 3 items. Be selective.
# - Once something moves to "resolved", it stays there unless explicitly reopened.
# - "invalidated" is for items made irrelevant by new information.
# - If a field has no entries, use an empty list [] or empty string "".
...
# # ── Domain routing ─────────────────────────────────────────────────────────────
# TRACKER_PROMPTS = {
#     "document editing": TRACKER_SYSTEM_PROMPT_DOCUMENT,
#     "math tutoring":    TRACKER_SYSTEM_PROMPT_MATH,
# }
...
# # ── User template (Fix 1 applied here later) ──────────────────────────────────
...
# Instructions:
# 1. Look at the last ASSISTANT message and move each item from previous explicit_needs and latent_needs to resolved if the assistant covered ot partially or fully.
...
#     Bumped to 1500 chars to handle math solutions without truncation.
#     """
#     lines = []
#     for turn in turns:
...
#     previous_state: dict = None,          # Fix 2: accept previous state
# ) -> dict:
#     """
...
#     # Fix 1 applied here: use stateful template when previous state exists
...
#     intent_states  = []
#     turns_so_far   = []
#     previous_state = None                  # Fix 3: thread previous state
...
#                 client=client,
#                 task_desc=task_desc,
#                 previous_state=previous_state,     # Fix 3
#             )
...
#                 "intent_state": state,
#             })
#             previous_state = state                 # Fix 3: update for next turn
...
# ── Config ─────────────────────────────────────────────────────────────────────
TRACKER_MODEL_ID = "openai.gpt-oss-20b-1:0"
AWS_REGION       = "us-east-1"
...
DIMENSIONS = {
    "document editing": {
        ...
        "dimensions": "goal intent, task structure, constraint awareness, missing information, audience fit, coherence, publication context, copyright and attribution",
    },
    "math tutoring": {
        "domain_desc":  "math tutoring",
        "domain_short": "math tutoring",
        "dimensions": "prerequisite knowledge gaps, conceptual vs procedural confusion, solution approach selection, common error patterns, hint vs full solution, generalization of technique",
    },
    "code generation": {
